...
- If no matching chat exists, call `import_review_chat` with the complete role-labeled transcript after every link has been rewritten. Pass the confirmed `source_ids` so the imported chat retains its library associations.
...
- If a matching chat needs changes but the live connector cannot update or append to that session, stop and explain the limitation. Do not call `import_review_chat` without `session_id`, because that creates a duplicate chat. Do not try to rewrite a completed import with `end_review_session`.
