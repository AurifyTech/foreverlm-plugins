...
Use the ForeverLM MCP server for every library, Project, Schedule, and review operation. Never infer stored state or substitute direct database access when the plugin fails.

1. Call `get_mcp_status` before starting a ForeverLM workflow. If no authorized hosted connection or active Mac relay is available, or the plugin returns an error, explain the plugin failure instead of inventing library state.
